...
# Config options
# Required https://github.com/altdesktop/playerctl
...
def scroll():
    ...
    if not status_paused:
        if visual_len(display_text) > message_display_len:
            ...
            # display_text = " "*(message_display_len - len(display_text)) + display_text
...
def print_text():
    ...
    if len(display_text) < message_display_len:
        print(
            ...
            + " "
            + "%%{T%d}" % (font_index)
            + make_visual_len(display_text, len(display_text))
            + "%{T-}"
            + display_suffix,
            flush=True,
        )
